...
def generate_code(model, tokenizer, test_dataloader, gold_column, id_labels, max_length):
    model.eval()
    outputs = []
    targets = []
    ids = {}
...
def model_eval(
    ...
    humaneval_results = humaneval_accuracy_score(
        data=results_df, 
        code_column_name=code_column) if compute_humanval else {}
    
    bleu_results = bleu_accuracy_score(
        data=results_df, 
        generated_column=code_column, 
        gold_column=gold_column) if compute_bleu else {}
...
def eval_generated_code(
    ...
    eval_df = generate_code(model, dataloader=dataloader, gold_column=target_label)
